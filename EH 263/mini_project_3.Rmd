...
gtsummary::tbl_summary(hobo_long, by = metric,
...
tbl_strata(data = indoor_outdoor %>% 
  pivot_longer(names_to = "in_out", values_to = "value", indoor_value:outdoor_value), 
           strata = metric,
           .tbl_fun = ~.x %>%
             tbl_summary(
               by = cooking, 
...
indoor_outdoor %>%
  group_by(metric) %>% 
  summarize(wilcox.test = tidy(wilcox.test(indoor_value,outdoor_value, paired = TRUE))) %>% 
  unnest(wilcox.test) %>% 
  flextable()%>% 
  flextable::add_footer_lines(values = "Table S2: Output from Wilcoxon signed rank test with continuity correction comparing temperature and relative humidity indoor and outdoors.")
